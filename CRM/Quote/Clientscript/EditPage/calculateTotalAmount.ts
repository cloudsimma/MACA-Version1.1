...
var margin_map = '{"A":0.50,"B":0.55,"C":0.60,"D":0.65,"E":0.70,"F":0.75,"G":0.80,"H":0.85,"X":1.00}';
...
{
    var Prix_Total = ((marginmp[quotesItem[j].Margin] / marginmp[quotesItem[j].Margin_B]) * quotesItem[j].Unit_Price_CA).toFixed(2);
    disTotalItem = ((marginmp[quotesItem[j].Margin] / marginmp[quotesItem[j].Margin_B]) * quotesItem[j].Unit_Price_CA).toFixed(2);
...
for (var i = 0; i < quotesdet.length; i++)
{
    ...
    {
        var Extra_Prix_Total = ((marginmp[quotesdet[i].Cost_Code] / marginmp[quotesdet[i].Margin_B]) * quotesdet[i].Unit_Price).toFixed(2);
              disTotalExtras = ((marginmp[quotesdet[i].Cost_Code] / marginmp[quotesdet[i].Margin_B]) * quotesdet[i].Unit_Price).toFixed(2);
        ...
        {
            var ExtrasTotal_Amount = ExtrasTotal_Amount + quotesdet[i].Quantity * Extra_Prix_Total;
            lineExtrasamount = quotesdet[i].Quantity * Extra_Prix_Total;
        }
    }
...
//console.log("lineItem2 ", lineExtrasPayload);
